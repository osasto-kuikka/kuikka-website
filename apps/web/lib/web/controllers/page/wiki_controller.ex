defmodule Web.Page.WikiController do
  use Web, :controller
  ...
  def index(conn, %{"editor" => "true"}) do
    if User.has_permission?(conn.assign.current_user, "read_wiki") do
      conn
      |> assign(:page, nil)
      |> assign(:content, "")
      |> render("editor.html")
  ...
  def index(conn, _params) do
    if User.has_permission?(conn.assigns.current_user, "read_wiki") do
